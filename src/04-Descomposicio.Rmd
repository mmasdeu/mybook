...
Calculem el cos de descomposició del poinomi $x^n-1$.
...
Tenim la torre $L / \QQ(\zeta_p) / \Q$ i $L/\QQ(\zeta_p)$ té grau com a molt $p$,
...
\xymatrix{
  & \QQ(\sqrt[p]{2},\zeta_p) \\
  \QQ(\zeta_p)\ar@{-}^{p}[ur]\ar@{-}^{p-1}[dr] &&\QQ(\sqrt[p]{2})\ar@{-}[ul]^{p-1}\ar@{-}[dl]^{p}\\
  &\QQ
}
\end{xy}
$$
...
:::
::: {.proof}
Consideremm un conjunt $\mathcal{U} \supset F$ de cardinal estrictament superior a $\mathcal{N}=\max(\aleph_0, |K|)$.
