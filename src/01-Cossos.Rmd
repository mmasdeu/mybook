...
Sigui $f(x)\in F[x]$ un polinomi irreductible, i suposem que $K/F$ és una extensió que conté una arrel $\alpha$ de $f(x)$.
...
$$
\Hom_{\sigma}(F(\alpha), L) \to \{\beta\in L ~|~ \sigma(f)(\beta) = 0 \}.
$$
$$
\xymatrix{
  F(\alpha)\ar^{\varphi}[r]\ar@{-}[dr] & L\\
  &F \ar{\sigma}[ur].
}
$$
:::
::: {.proof}
$$
\Hom_\sigma (F(\alpha), L) \cong \Hom_{\sigma}(F[x]/(f(x)), L) \cong \{ \sigma \in \Hom_{\sigma}(F[x],L) ~|~ \sigma(f) = 0\},
$$
