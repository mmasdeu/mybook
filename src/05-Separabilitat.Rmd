...
::: {.lemma}
Sigui $f(x)\in F[x]$ amb  $\char(F)=p$. Si $f'(x)=0$, aleshores hi ha un polinomi $f_1[x]\in F[x]$ tal que $f(x)=f_1(x^p)$.
:::

::: {.proposition}
Sigui $f(x)\in F[x]$ amb $\char(F)=p$ un polinomi irreductible. Aleshores hi ha un únic $k\geq 0$ i un únic
...
Sigui $f(x)\in F[x]$ amb $\char(F)=p$ un polinomi irreductible. El *grau de separabilitat* de $f(x)$
...
:::
::: {.proof}
Només cal veure que $(a+b)^p = a^p + b^p$ i que $(ab)^p = a^p b^p$. La primera igualtat es veu fent servir que $p$ divideix a $\binomial{p}{i}$
...
Frobenius és un isomorfisme (només cal comptar), però en general no és cert. Per exemple, la imatge de Frobenius a $F=\FF_p(t)$ és $\FF_p(t^p)$.

::: {.proposition name=""}
Si $\FF$ és un cos finit, aleshores tot polinomi irreductible sobre $\FF$ és separable.
:::
::: {.proof}
...
::: {.corollary}
Tota extensió finita d'un cos perfecte és separable.
:::


## Cossos finits

::: {.proposition name="Existència i unicitat de cossos finits"}
