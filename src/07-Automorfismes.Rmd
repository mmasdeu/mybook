...
::: {.theorem name="Caracterització d'extensions de Galois"}
...
indicat a la Remarca @ref(remark:variacions-aixecament), a cada pas podem prendre per $\alpha$ una arrel del polinomi $f(x)$
...
Si $K/F$ és una extensió de Galois i $\alpha\in K$, els elements $\sigma(\alpha)$ (on $\sigma\in\Gal(K/F)$) s'anomenen
*conjugats de Galois* d'$\alpha$ sobre $F$. Si $K/M/F$ és una subextensió, el cos $\sigma(M)$ s'anomena el conjugat de $M$ per $\sigma$.